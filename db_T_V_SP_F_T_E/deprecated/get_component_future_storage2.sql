...
DROP FUNCTION IF EXISTS `get_component_future_storage2`$$

CREATE DEFINER=`root`@`%` FUNCTION `get_component_future_storage2`(comp_id VARCHAR(255), _useFutureStorage INT) RETURNS INT(11)
BEGIN
    DECLARE total INT;
