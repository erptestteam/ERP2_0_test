...
DROP PROCEDURE IF EXISTS `order_disassemble5`$$

CREATE DEFINER=`root`@`%` PROCEDURE `order_disassemble5`(IN _useActualStorage INT,IN _useFutureStorage INT,
...
BEGIN
    ...
    IF _found THEN
        ...
        WHILE _found DO
            ...
            SET _actualStorage = (SELECT get_component_actual_storage2(_sub_c_actual_storage,_useActualStorage));
            SET _futureStorage = (SELECT get_component_future_storage2(_sub_c_future_storage,_useFutureStorage));
