...
DROP FUNCTION IF EXISTS `f_get_component_actual_storage`$$

CREATE DEFINER=`root`@`%` FUNCTION `f_get_component_actual_storage`(comp_id VARCHAR(255), _useActualStorage INT) RETURNS INT(11)
BEGIN
    DECLARE total INT;
